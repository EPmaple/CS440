...
/*
        private void countProbs(Matrix X, Matrix y_gt) {
            int XnumRows = X.getShape().getNumRows();
            int XnumCols = X.getShape().getNumCols();

            for (int i = 0; i < XnumRows; i++) {
                for (int j = 0; j < XnumCols; j++) {
                    // j=0, j=1 are continuous, j=2, j=3 are discrete
                    double element = X.get(i, j);
                    if (j == 0) {
                        continuous1Sum += element;
                        continuous1[2] += 1.0;
                    } else if (j == 1) {
                        continuous2Sum += element;
                        continuous2[2] += 1.0;
                    } else if (j == 2) {
                        double outerKey = y_gt.get(i, 0);
                        double innerKey = X.get(i, j);
                        updateNestedMap(outerKey, innerKey, discrete3);

                    } else if (j == 3) {
                        double outerKey = y_gt.get(i, 0);
                        double innerKey = X.get(i, j);
                        updateNestedMap(outerKey, innerKey, discrete4);
